...
//Modified for use with Atlas Scientific tripump
...
const String ssid = "UD Devices";                                 //The name of the Wi-Fi network you are connecting to
const String pass = "";                             //Your WiFi network password
const long myChannelNumber = 2186484;                            //Your Thingspeak channel number
const char * myWriteAPIKey = "URQC2HBIU9LBXNF0";                 //Your ThingSpeak Write API Key
//------------------------------------------------------------------
...
Ezo_board PMP1 = Ezo_board(56, "PMP1");    //create an PMP circuit object who's address is 103 and name is "PMP"
Ezo_board PMP2 = Ezo_board(57, "PMP2");  
Ezo_board PMP3 = Ezo_board(58, "PMP3");

int nutrient_counter_a = 0;
int nutrient_counter_b = 0;
int acid_counter = 0;
...
void pump_function(Ezo_board &pump, Ezo_board &sensor, float value, float dose,  bool greater_than) {
  if (sensor.get_error() == Ezo_board::SUCCESS) {                    //make sure we have a valid reading before we make any decisions
    ...
    if (true) {                                              //if the result of the comparison means we should activate the pump
      pump.send_cmd_with_num("d,", dose);                           //dispense the dose
      delay(100);
      // PMP2.send_cmd_with_num("d,", dose);                                                   //wait a few milliseconds before getting pump results
...
void step4() {
  ...
  if (EC.get_error() == Ezo_board::SUCCESS) {                                           //if the EC reading was successful (back in step 1)
    ThingSpeak.setField(2, String(EC.get_last_received_reading(), 2));                 //assign EC readings to the second column of thingspeak channel
  }
  Serial.print("  ");
...
//Serial.print("nut_b doses: ");
//Serial.println(nutrient_counter_b);
